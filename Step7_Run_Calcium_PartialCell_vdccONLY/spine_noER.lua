...
function neumannBndCa(x, y, z, t, si)
	...
	if 	t <= caEntryDuration then
		influx = 0.0001 * (1.0 - t/caEntryDuration)
	else
		influx = 0.0
	end
...
vdccMode = 1  -- 0 for script function voltage, 1 for data file voltage
if vdccMode == 0 then
...
elseif vdccMode == 1 then
	print("Using voltage data files")
...
--- non-linear solver ---
-- convergence check
newtonConvCheck = CompositeConvCheck(approxSpace, 10, 1e-10, 1e-08)
